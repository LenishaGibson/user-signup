...
def index():
    ...
    if request.method == 'POST':
        ...
        for i in username:
            #if there is a blank space in username then it's invalid/deny
            if i.isspace():
...
#If all goes well in login requirements, show welcome + username
@app.route('/confirmation')
def confirmation():
    title = "Welcome!"
    username = request.args.get('username')
    encoded_error = request.args.get("error")
    return render_template('confirmation.html', title=title, username=username, error=encoded_error and cgi.escape(encoded_error, quote=True))
